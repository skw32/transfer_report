...
Refer to: CH 5 (other PV materials) + pg 187 (future of PV) \cite{PV_Goertzberger}\\
...
\begin{figure}[h!]
  \centering
    ...
    \caption{AM1.5 solar spectrum with distinct dips due to molecular absorption in Earth’s atmosphere. The dashed line shows the band gap of silicon and fading indicates that photons with energies below the band gap will not be absorbed. Figure taken from reference \citenum{newPVrev}.}
...
Computational studies can be used to screen for candidate materials that may have a desirable property to determine those worthy of further study by either more advanced computational techniques or experimentally \cite{high_tp2}.\\
\begin{figure}[h!]
  \centering
...
There is a similar ideology behind this small study where we perform a similar screening process on a much smaller scale. For our study, the database is a book containing data on over 200 naturally occurring minerals, given in reference \citenum{DK}, and the descriptors for candidate materials with the property of interest, photoferroelectricity, are based on the colour of the mineral and the crystal structure. Photoferroelectric materials, also referred to as photoferroics, are ferroelectric materials that are able to absorb sunlight, ideally in the visible range. A dark-coloured material suggests that it absorbs light in the visible range. This point is discussed in further detail in section \ref{colour}. Ferroelectric materials are a subset of materials with polar crystal structures, therefore not all polar materials exhibit ferroelectricity. It is only polar materials that possess a spontaneous electric dipole moment within the unit cell that can be inverted by the application of an external electric field that exhibit ferroelectricity \cite{FE_subset}. Polar dielectric materials and ferroelectricity are discussed further in section \ref{dielectrics}.
...
From this screening process, we identified three candidate materials that could exhibit ferroelectricity based on our screening criteria; these are: enargite ($Cu_{3}AsS_{4}$), stephanite ($Ag_{5}SbS_{4}$) and bournonite ($PbCuSbS_{3}$). All three minerals are sulfosalts, which are materials that contain two or more metals, semi-metals such as antimony and arsenic, and sulfur \cite{DK}. Known properties of the three candidate materials are described in section \ref{known_prop}.\\

This study aims to identify materials that could exhibit ferroelectricity, but also possess band gaps that are better-matched to the solar spectrum than currently well-known ferroelectric materials.
The discovery of such materials could enable phenomena related to ferroelectricity, such as the APE and BPE, to be exploited for high efficiency solar cells. As highlighted in the section above, our initial screening criteria for photoferroelectricity in our dataset of minerals are necessary but not sufficient conditions for this desirable property. Further investigation of the three candidate materials is therefore necessary to determine if any do in fact exhibit ferroelectricity. 
